  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: HeaderWidget(title: title, scaffoldKey: key, actions: actions),
      ),
      drawer: AppDrawer(scaffoldKey: key),
      body: Column(children: [Expanded(child: body), const FooterWidget()]),
      bottomNavigationBar: bottomNavigationBar,
      floatingActionButton: Navigator.of(context).canPop()
          ? FloatingActionButton(onPressed: () => Navigator.pop(context), backgroundColor: const Color(0xFFFF3131), child: const Icon(Icons.arrow_back, color: Colors.white))
          : null,
  ...
  Widget build(BuildContext context) {
    ...
    return Drawer(
      child: ListView(padding: EdgeInsets.zero, children: [
        DrawerHeader(
          decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFFF3131), Color(0xFFFF8C00), Color(0xFFFFDE59)])),
        ...
        ListTile(leading: const Icon(Icons.book, color: Color(0xFFFF3131)), title: const Text('المواد'), onTap: () {
          Navigator.pop(context);
          final u = FirebaseAuth.instance.currentUser;
          if (u != null) FirebaseFirestore.instance.collection('users').doc(u.uid).get().then((doc) {
            Navigator.push(context, MaterialPageRoute(builder: (_) => StudentHomePage(specialty: doc.get('specialty') ?? '', year: doc.get('year') ?? '')));
          });
        }),
        ListTile(leading: const Icon(Icons.settings, color: Color(0xFFFF8C00)), title: const Text('الإعدادات'), onTap: () {
          Navigator.pop(context);
          Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsPage()));
        }),
        ListTile(leading: const Icon(Icons.info, color: Color(0xFFFFDE59)), title: const Text('حول التطبيق'), onTap: () {
          Navigator.pop(context);
        ...
        const Divider(),
        const Padding(padding: EdgeInsets.all(16), child: Text('تابعنا', style: TextStyle(fontWeight: FontWeight.bold))),
        StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            return Column(children: snapshot.data!.docs.map((doc) {
              ...
              return ListTile(
                leading: Icon(_icons[data['platform']] ?? Icons.link),
