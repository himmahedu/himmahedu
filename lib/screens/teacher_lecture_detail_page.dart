...
class _TeacherLectureDetailPageState extends State<TeacherLectureDetailPage> {
  ...
  Future<void> _updateMeetLink() async {
    await FirebaseFirestore.instance.collection('courses').doc(widget.courseId).update({'meetLink': _meetCtrl.text.trim()});
                  ...
                  children: lectures.map((doc) {
                    ...
                    return ListTile(
                      ...
                      trailing: Row(
                        ...
                        children: [
                          IconButton(icon: const Icon(Icons.quiz, color: Color(0xFFFFDE59)), onPressed: () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => TeacherQuizEditorPage(lectureId: doc.id)));
                          }),
                          IconButton(icon: const Icon(Icons.people, color: Color(0xFFFF8C00)), onPressed: () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => TeacherStudentsListPage(lectureId: doc.id)));
                          }),
                          IconButton(icon: const Icon(Icons.chat, color: Color(0xFFFF3131)), onPressed: () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => ChatPage(courseId: widget.courseId, courseName: widget.courseName)));
                          }),
